...
class Bharatx_Pay_In_3_Feature_Gateway extends WC_Payment_Gateway {
	...
	//	$this->category_ids			  = $this->get_option( 'category_ids' );
		...
		add_action( 'woocommerce_api_' . strtolower( 'BharatX_Pay_In_3_Feature_Gateway' ), array( $this, 'webhook_callback' ) );
	...
	public function webhook_callback() {
		...
		try {
			...
			if ( isset( $_POST['txnStatus'] ) && 'SUCCESS' === sanitize_text_field( wp_unslash( $_POST['txnStatus'] ) ) ) {

				$status              = ( isset( $_POST['txnStatus'] ) ) ? sanitize_text_field( wp_unslash( $_POST['txnStatus'] ) ) : '';
				$nonce               = ( isset( $_POST['nonce'] ) ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';
				$transaction_id      = ( isset( $_POST['txnId'] ) ) ? sanitize_text_field( wp_unslash( $_POST['txnId'] ) ) : '';
				...
				$error   = isset( $_POST['error_code'] ) ? sanitize_text_field( wp_unslash( $_POST['error_code'] ) ) : '';
				$message = esc_html__( 'Your payment via BharatX was Cancelled. Please try again.' , 'Bharatx_Pay_In_3_Feature_Plugin' );
				$order->update_status( 'failed', $message );
				$this->add_order_notice( $message );
			}
